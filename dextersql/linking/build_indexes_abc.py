...
# `build_indexes` is a sibling module imported by bare name (the callers in this
# package historically put this directory on sys.path before importing here).
# Insert it ourselves too, so this module is also importable directly as
# `dextersql.linking.build_indexes_abc` without a caller having set the path up.
_HERE = Path(__file__).resolve().parent
if str(_HERE) not in sys.path:
    sys.path.insert(0, str(_HERE))

from build_indexes import get_embeddings  # noqa: E402
