...
class LLMConfig(BaseModel):
    ...
    base_url: Optional[str] = Field(default=None, description="The base url of the model service (not required for vllm)")
    api_key: Optional[str] = Field(default=None, description="The api key of the model service (not required for vllm)")
    ...
    api_type: Literal["openai", "azure", "vllm", "gpt5_oss", "gpt5"] = Field(default="openai", description="The type of the api")
    ...
    # vLLM-specific fields (ignored when api_type != "vllm")
    vllm_tensor_parallel_size: int = Field(default=-1, description="Number of GPUs for tensor parallelism; -1 = auto-detect from CUDA_VISIBLE_DEVICES")
    vllm_quantization: Optional[str] = Field(default="mxfp4", description="vLLM quantization method: mxfp4 | fp8 | awq | gptq | None")
    vllm_gpu_memory_utilization: float = Field(default=0.90, description="Fraction of GPU VRAM vLLM may use")
    vllm_max_model_len: Optional[int] = Field(default=32768, description="Max sequence length override for vLLM (None = model default)")

    @model_validator(mode="after")
    def _validate_api_credentials(self):
        if self.api_type in ("openai", "azure", "gpt5_oss", "gpt5"):
            if not self.base_url:
                raise ValueError(f"base_url is required when api_type='{self.api_type}'")
            if not self.api_key:
                raise ValueError(f"api_key is required when api_type='{self.api_type}'")
        return self
...
class SQLRuleCorrectionConfig(BaseModel):
    llm: LLMConfig = Field(..., description="The llm config, used to apply rule-guided sql correction")
    n_parallel: int = Field(default=16, description="The number of parallel threads to use")
    n_internal_parallel: int = Field(default=16, description="Max parallel workers within a single sample (correcting unique candidates)")
    save_path: str = Field(default=_path_to_str(WORKSPACE_ROOT / "sql_rule_correction"), description="The save path of the sql rule correction result")
    rules_path: str = Field(default="", description="JSON rule set to use (e.g. rule_creator's 05_created_rules.json); empty = the shipped example rules")
...
class SchemaExplorationConfig(BaseModel):
    """Fully optional. enabled=False (the default) means the pipeline runs
    exactly as if this config section didn't exist -- see
    dextersql.schema_exploration and the single call site in pipeline.py."""
    enabled: bool = Field(default=False, description="Inject offline-mined column-disambiguation notes into evidence after schema_linking")
    notes_dir: str = Field(default=_path_to_str(WORKSPACE_ROOT / "schema_exploration" / "notes"), description="Directory of offline-mined ambiguity_notes_<db_id>.json files, one per database")
    gate_llm: Optional[LLMConfig] = Field(default=None, description="LLM for the online per-question relevance gate; required only when enabled=True")
    max_notes_per_question: int = Field(default=0, description="Cap on notes the gate may keep per question (0 = unlimited)")
...
class AppConfig(BaseModel):
    ...
    sql_rule_correction: SQLRuleCorrectionConfig = Field(default_factory=SQLRuleCorrectionConfig, description="The config of the sql rule correction")
    sql_selection: SQLSelectionConfig = Field(default_factory=SQLSelectionConfig, description="The config of the sql selection")
    schema_exploration: SchemaExplorationConfig = Field(default_factory=SchemaExplorationConfig, description="Optional column-disambiguation note injection; disabled unless explicitly configured")
...
class Config:
    ...
    def _initialize_config(self, config_path: Optional[Path] = None):
        ...
        # sql rule correction config
        sql_rule_correction_config = config.get("sql_rule_correction", {})
        sql_rule_correction_settings = {
            "llm": LLMConfig(**sql_rule_correction_config.get("llm")),
            "n_parallel": sql_rule_correction_config.get("n_parallel", 16),
            "n_internal_parallel": sql_rule_correction_config.get("n_internal_parallel", 16),
            "save_path": _path_to_str(sql_rule_correction_config.get("save_path", WORKSPACE_ROOT / "sql_rule_correction")),
            "rules_path": sql_rule_correction_config.get("rules_path", ""),
        }
        ...
        # schema exploration config -- FULLY OPTIONAL. An absent
        # [schema_exploration] section (every config used before this
        # feature existed, and every config that doesn't opt in) parses to
        # enabled=False with no other section required; only enabled=true
        # needs a [schema_exploration.gate_llm] sub-section.
        schema_exploration_config = config.get("schema_exploration", {})
        se_enabled = schema_exploration_config.get("enabled", False)
        se_gate_llm_raw = schema_exploration_config.get("gate_llm")
        schema_exploration_settings = {
            "enabled": se_enabled,
            "notes_dir": _path_to_str(schema_exploration_config.get("notes_dir", WORKSPACE_ROOT / "schema_exploration" / "notes")),
            "gate_llm": LLMConfig(**se_gate_llm_raw) if se_gate_llm_raw else None,
            "max_notes_per_question": schema_exploration_config.get("max_notes_per_question", 0),
        }
        if se_enabled and schema_exploration_settings["gate_llm"] is None:
            raise ValueError("schema_exploration.enabled=true requires a [schema_exploration.gate_llm] section")
        ...
        self._app_config = AppConfig(
            ...
            sql_rule_correction=SQLRuleCorrectionConfig(**sql_rule_correction_settings),
            sql_selection=SQLSelectionConfig(**sql_selection_settings),
            schema_exploration=SchemaExplorationConfig(**schema_exploration_settings),
    ...
    @property
    def sql_rule_correction_config(self):
        return self._app_config.sql_rule_correction
    ...
    @property
    def schema_exploration_config(self):
        return self._app_config.schema_exploration
