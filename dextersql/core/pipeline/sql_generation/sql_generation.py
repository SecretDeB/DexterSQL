...
from .generators import DCGenerator, ICLGenerator
from dextersql.core.pipeline.validation import validate_pipeline_step
import time
from dextersql.core.logger import logger
from tqdm import tqdm
import traceback
from dextersql.core.services import ArtifactStore, STAGE_ARTIFACT_FIELDS, configure_schema_service, load_stage_dataset, reset_schema_service
...
class SQLGenerationRunner:
    ...
    # No stock type here: install_dep_tree_generator() binds "SkeletonGenerator"
    # into this module's namespace at runtime, before __init__ ever runs.
    _skeleton_generator = None
    ...
    # An item can come back with zero candidates when every generator fails at once
    # -- typically a reasoning model spending its whole completion budget before
    # emitting <result>, which is most likely on large schemas. That is transient,
    # so retry those items in-run rather than leaving the stage incomplete.
    EMPTY_RETRY_ROUNDS = 2

    def _submit_all(self, items, desc):
        """Generate for `items`, recording each as it completes."""
        future_to_item = {}
        for data_item in items:
            future = self._thread_pool_executor.submit(self._generate_sql, data_item)
            future_to_item[future] = data_item
        for idx, future in tqdm(enumerate(as_completed(future_to_item), start=1),
                                total=len(future_to_item), desc=desc):
            future.result()
            self._artifact_store.record_item(future_to_item[future])
            if idx % 5 == 0:
                logger.info(f"{desc} {idx} / {len(future_to_item)} completed")
                self.save_result()

    @staticmethod
    def _is_empty(data_item) -> bool:
        return not getattr(data_item, "sql_candidates", None)

    def run(self):
        pending = [it for it in self._dataset if not it.is_stage_complete("sql_generation")]
        skipped = len(list(self._dataset)) - len(pending)
        if skipped:
            logger.info(f"Skipping {skipped} items already generated")

        self._submit_all(pending, "Generating SQL")

        # Retry rounds for items that produced nothing at all.
        for attempt in range(1, self.EMPTY_RETRY_ROUNDS + 1):
            empty = [it for it in self._dataset if self._is_empty(it)]
            if not empty:
                break
            ids = [it.question_id for it in empty]
            logger.warning(
                f"{len(empty)} item(s) produced NO candidates "
                f"(retry {attempt}/{self.EMPTY_RETRY_ROUNDS}): {ids[:20]}"
                + (" ..." if len(ids) > 20 else ""))
            self._submit_all(empty, f"Retry {attempt} (empty)")

        still_empty = [it.question_id for it in self._dataset if self._is_empty(it)]
        if still_empty:
            # Surface loudly: these will fail the stage validation below, and the
            # cause is here, not in the stage that consumes the output.
            logger.error(
                f"{len(still_empty)} item(s) STILL have no candidates after "
                f"{self.EMPTY_RETRY_ROUNDS} retries: {still_empty[:20]}"
                + (" ..." if len(still_empty) > 20 else ""))
