...
Runs value_retrieval, then generation/revision/correction/selection, from a raw
dataset to a scored result:
    ...
    4. sql_rule_correction apply the fixed semantic correction rule set (see
                           dextersql/rule_correction/) -- ships with 2 example
                           rules and uses them by default; see
                           dextersql/rule_creator/ to mine a rule set from your
                           own training data instead
    5. sql_selection       pick one answer by confidence-aware pairwise voting
...
"Run the whole pipeline" section for the full sequence. The same is true of the
optional column-disambiguation note injection in `dextersql/schema_exploration/`:
it is an explicit step run between schema linking and sql_generation (see that
module's README), not a stage of this class, and does nothing unless you run it.
...
STAGES = ["value_retrieval", "sql_generation", "sql_revision",
          "sql_rule_correction", "sql_selection"]
