...
def make_backend(kind: str,
    ...
    kind:
      ...
      - "openai"            -> OpenAIBackend (API; also covers OpenRouter and any
                               other OpenAI-compatible endpoint via base_url)
    ...
    raise ValueError(
        ...
        f"Use 'qwen', 'gptoss', 'gptoss120b', 'vllm', 'vllm_server', or 'openai'. "
        f"(For an OpenRouter-hosted model, use 'openai' with base_url set to "
        f"OpenRouter's OpenAI-compatible endpoint.)"
    )
