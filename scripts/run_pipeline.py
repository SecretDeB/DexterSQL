...
Run the DexterSQL pipeline: dataset in, predictions out.
    ...
    value_retrieval -> sql_generation -> sql_revision -> sql_rule_correction -> sql_selection
...
only covers value_retrieval and the generation/revision/correction/selection tail,
and sql_generation always uses dep_tree as its third generator.
sql_rule_correction applies the example rules in dextersql/rule_correction/
example_rules.py by default -- see the README's "Rule-guided correction" section
for how to mine a rule set of your own instead.
...
Examples
--------
    # value_retrieval, then generation/revision/correction/selection
    ...
    python scripts/run_pipeline.py --config config/bird.toml \
        --stages sql_revision sql_rule_correction sql_selection
