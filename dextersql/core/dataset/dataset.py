...
from typing import ClassVar, TYPE_CHECKING, List, Optional, Any, Dict
...
if TYPE_CHECKING:
    from dextersql.core.config import DatasetConfig
...
class DataItem(BaseModel):
    ...
    # SQL Rule Correction Step
    sql_candidates_after_rule_correction: Optional[List[str]] = Field(default=None, description="The sql candidates after rule-guided correction of the data item")
    rule_correction_applied_rules: Optional[Dict[str, List[str]]] = Field(default=None, description="Map of normalized candidate sql -> applied correction rule names, for candidates that were changed")
    ...
    rule_correction_time: Optional[float] = Field(default=None, description="The time cost of sql rule correction of the data item")
    ...
    rule_correction_llm_cost: Optional[Dict[str, Any]] = Field(default=None, description="The llm cost of sql rule correction of the data item")
    ...
    def get_pipeline_artifacts(self) -> PipelineArtifacts:
        return PipelineArtifacts(
            ...
            sql_rule_correction=self.get_stage_artifact("sql_rule_correction"),
    ...
    def apply_pipeline_artifacts(self, pipeline_artifacts: PipelineArtifacts | Dict[str, Any]) -> None:
        ...
        self.apply_stage_artifact("sql_rule_correction", pipeline_artifacts.sql_rule_correction)
    ...
    # Stage fields that are only meaningful when NON-EMPTY. A stage that produced an
    # empty list did not really succeed: sql_generation returning [] means every
    # generator failed for that item (a reasoning model can burn its entire
    # completion budget before emitting <result>, which happens most on large
    # schemas). Treating [] as "done" silently strands the item -- it is skipped on
    # every resume, and the NEXT stage then fails validation on a None field far
    # from the real cause. Requiring non-empty makes such items retry naturally.
    NON_EMPTY_STAGE_FIELDS: ClassVar[Dict[str, tuple]] = {
        "sql_generation": ("sql_candidates",),
        "sql_revision": ("sql_candidates_after_revision",),
        "sql_rule_correction": ("sql_candidates_after_rule_correction",),
    }

    def _stage_field_incomplete(self, artifact, stage_name: StageName, field_name: str) -> bool:
        value = getattr(artifact, field_name)
        if value is None:
            return True
        if field_name in self.NON_EMPTY_STAGE_FIELDS.get(stage_name, ()):
            try:
                return len(value) == 0
            except TypeError:
                return False
    ...
    def is_stage_complete(self, stage_name: StageName) -> bool:
        ...
        if any(self._stage_field_incomplete(artifact, stage_name, field_name)
    ...
    def get_stage_validation_errors(self, stage_name: StageName) -> List[Dict[str, str]]:
        ...
        for field_name in STAGE_VALIDATION_FIELDS[stage_name]:
            ...
            elif self._stage_field_incomplete(artifact, stage_name, field_name):
                errors.append({"field": field_name,
                               "error": f"Field '{field_name}' is empty (no candidates produced)"})
...
class BaseDataset(ABC):
    ...
    def _load_database_schema(self, database_id: str):
        from dextersql.core.services import get_schema_service
